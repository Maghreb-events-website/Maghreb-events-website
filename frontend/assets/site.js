...
ME.VATSIM = {
  CLIENT_ID:    '1383',                // ← REPLACE with your auth-dev.vatsim.net client id
  AUTHORIZE:    'https://auth-dev.vatsim.net/oauth/authorize',  // VATSIM Connect Sandbox
  REDIRECT_URI: window.location.protocol === 'file:'
    ? 'https://jamie-datson.com/callback.html'
...
ME.auth = {
  ...
  logout() { localStorage.removeItem('me_token'); localStorage.removeItem('me_user'); window.location.href = 'index.html'; },
  normalizeUser(user) {
...
ME.nav = {
  links: [
    ...
    { href: 'airlines.html', label: 'Airlines',      key: 'airlines' },
    { href: 'planning.html', label: 'Planning Team', key: 'planning' },
    { href: 'hitsquad.html', label: 'Hitsquad',      key: 'hitsquad' },
    { href: 'briefing.html', label: 'Pilot Briefing', key: 'briefing' },
  ],
  inject(activeKey) {
    ...
    let cta;
    if (isAdmin) {
      cta = `<a href="admin.html" class="me-nav-cta">Admin Panel</a>
      ...
      cta = `<a href="login.html" class="me-nav-cta">Login</a>`;
    }
    ...
    const header = `
      <header class="me-header">
        <a class="me-brand" href="index.html">
